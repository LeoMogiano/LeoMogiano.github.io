import type { Dict } from './types';

export const ja: Dict = {
  code: "ja-JP",
  langLabel: "JA",
  langNext: "Español",
  langAria: "言語を切り替える",
  themeAria: "表示モードを切り替える",
  role: "モバイル・テックリード",
  ogImageAlt: "Leo Mogiano、モバイル・テックリード — iOS、Android、Flutter",
  clockTitle: "ボリビアの現地時間",
  nav: ["ホーム", "アプリ", "経歴", "プレイ", "連絡"],
  photoAlt: "レオ・モヒアーノのポートレート",
  statusAria: "ステータス",
  statusTitle: "ビルド中",
  playAria: "再生",
  greetA: "こんにちは、",
  greetB: "です",
  bio: "良いアプリは、使い方を説明されなくても使える。まじめなアプリは、失敗したときに理由を伝える。この二つを基準に、フィンテック・デジタル本人確認・決済の分野でおよそ3年、5本のアプリを本番環境へ。現在は Datec Corp でモバイルチームをリードしています。",
  cvLabel: "職務経歴書",
  cvAria: "Leo Mogiano の履歴書を PDF でダウンロード",
  descDatecLead: "複数案件を並行する Flutter チームを統括。コード規約を定め、アーキテクチャを決め、リリースの責任を持つ。今も毎日コードを書いている。",
  descDatecMob: "1年で4つのプロダクト。Firebase と Sentry を使った EGX One のベータ、Kotlin のキャッシュレス自販機、NFC リストバンド、決済デモ。ほぼ全てが機器と通信するものだった。",
  descValidme: "分散型本人確認を一気通貫で開発。KYC・生体認証・暗号化を iOS と Android のネイティブ SDK 上で実装し、プッシュ通知と Isolate、Crashlytics での安定性監視まで。",
  descPresto: "Flutter と BLoC による金融アプリ、主にオンボーディング。Scrum・Gitflow・PR レビューを回し、Firebase、TestFlight、両ストアで配信。GitHub Actions の CI にも貢献。",
  descGetserver: "Flutter（Provider）のモバイルアプリと Laravel の Web システムを、教育系と配送管理の案件向けに開発。決済ゲートウェイを統合し、手作業の処理を自動化した。",
  descBanco: "Java の申立管理システムの保守。修正・改善・サポートを銀行の業務部門と進めた、人が毎日頼るソフトウェアに初めて関わった仕事。",
  workBadge: "アプリ",
  sectionAria: "セクション {n}／{total}：{label}",
  workA: "",
  workEm: "関わった",
  workB: "プロダクト 🚀",
  workNote: "各行をタップすると端末でプレビューできます。",
  pathBadge: "経歴",
  pathA: "",
  pathEm: "働いてきた",
  pathB: "場所 🛠️",
  roleLead: "モバイル・テックリード",
  roleMob: "モバイル開発者",
  roleMobWeb: "モバイル・Web開発者",
  roleAnalyst: "システムアナリスト",
  now: "現在",
  durYear: "年",
  durYears: "年",
  durMonth: "ヶ月",
  durMonths: "ヶ月",
  durSep: "",
  playBadge: "プレイ",
  vs: "vs",
  playP: "聖戦は、スマホの中のポンで決着。画面の上で指かマウスを動かしてください。下のパドルがあなたです。",
  youIOS: "あなた · iOS",
  cpuAndroid: "CPU · Android",
  resetBtn: "スコアをリセット",
  pongAria: "ポンゲーム：iOS 対 Android。カーソルまたは矢印キーで操作します。",
  boatSub: "同じ舟でも、時間の流れは違う",
  ctaA: "",
  ctaEm: "次のアプリ",
  ctaB: "の話をしましょう",
  fromBolivia: "🇧🇴 ボリビアから、世界へ",
  eggCaption: "きみへ、いつまでも ❤️",
  eggClose: "タップで閉じる",
  marqueePause: "スクロールを一時停止",
  marqueePlay: "スクロールを再開",
  appSelected: "表示中",
  appPickerAria: "アプリを選ぶ",
  pongHelp: "左右の矢印キーでパドルを操作します。",
  shotsAria: "{app} のスクリーンショット",
  shotAlt: "{app} のスクリーンショット {n}",
  shotGo: "{app} のスクリーンショット {n} を表示",
  shotOpen: "{app} のスクリーンショットを全画面で表示",
  shotsFullAria: "{app} のスクリーンショット、全画面",
  shotClose: "閉じる",
};
