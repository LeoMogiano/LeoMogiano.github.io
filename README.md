...
```bash
bun run assets    # .vendor-assets/ -> src/assets/  (WebP verificado)
bun run fonts     # descarga los .ttf y genera public/fonts/*.woff2
node scripts/build-icons.mjs   # SVG -> src/styles/icons.css
```

`bun run assets` verifica lo que produce: los gráficos planos van a WebP sin
...
`bun run fonts` subsetea con harfbuzz. Las familias con versión variable se
